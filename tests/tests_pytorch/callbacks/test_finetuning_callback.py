...
def test_finetuning_callback(tmpdir):
    ...
    trainer = Trainer(
        limit_train_batches=4, default_root_dir=tmpdir, callbacks=[callback], max_epochs=8, accelerator="auto"
    )
    trainer.fit(model)
...
def test_finetuning_callback_warning(tmpdir):
    ...
    with pytest.warns(UserWarning, match="Did you init your optimizer in"):
        trainer = Trainer(
            limit_train_batches=1, default_root_dir=tmpdir, callbacks=[callback, chk], max_epochs=2, accelerator="auto"
        )
        trainer.fit(model)
...
def test_base_finetuning_internal_optimizer_metadata(tmpdir):
    ...
    model = FreezeModel()
    trainer = Trainer(
        default_root_dir=tmpdir, max_epochs=5, limit_train_batches=1, callbacks=[cb, chk], accelerator="auto"
    )
    trainer.fit(model)
    ...
    model = FreezeModel()
    cb = OnEpochLayerFinetuning()
    trainer = Trainer(max_epochs=10, callbacks=[cb], accelerator="auto")
