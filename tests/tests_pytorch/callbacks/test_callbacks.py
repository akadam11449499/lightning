...
def test_resume_callback_state_saved_by_type_stateful(tmpdir):
    ...
    trainer = Trainer(default_root_dir=tmpdir, max_steps=1, callbacks=[callback], accelerator="auto")
    trainer.fit(model)
    ckpt_path = Path(trainer.checkpoint_callback.best_model_path)
    assert ckpt_path.exists()

    callback = OldStatefulCallback(state=222)
    trainer = Trainer(default_root_dir=tmpdir, max_steps=2, callbacks=[callback], accelerator="auto")
